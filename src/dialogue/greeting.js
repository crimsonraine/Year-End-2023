class GreetingScene extends Phaser.Scene {
    ...
    create () {
        ...
        next_button.on('pointerdown', () => {
            if (i == 8) {
                this.scene.start('MapScene', {character : this.character});
            }
            else {
        ...
        if (this.character == 'A') {
            this.atelle = this.physics.add.sprite(860, 355, ('player_' + this.character + '_idle')).setScale(8);
        }
        else {
            this.atelle = this.physics.add.sprite(780, 355, ('player_' + this.character + '_idle')).setScale(8);
        }
